...
class SurvilDB:
    """Small local persistence layer; designed to migrate to PostgreSQL later."""
    ...
    def add_camera(self, name: str, source: str) -> int:
        with self.connect() as conn:
            cur = conn.execute(
                "INSERT INTO cameras(name, source, created_at) VALUES (?, ?, ?)",
                (name, source, utc_now()),
            )
    ...
    def add_person(self, name: str, external_ref: str | None = None) -> int:
        with self.connect() as conn:
            cur = conn.execute(
                "INSERT INTO people(name, external_ref, created_at) VALUES (?, ?, ?)",
                (name, external_ref, utc_now()),
            )
            return int(cur.lastrowid)
    ...
    def add_event(
        ...
        with self.connect() as conn:
            cur = conn.execute(
                """INSERT INTO events
                (camera_id, person_id, event_type, confidence, track_id,
                 snapshot_path, metadata_json, occurred_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (camera_id, person_id, event_type, confidence, track_id,
                 snapshot_path, metadata_json, utc_now()),
            )
            return int(cur.lastrowid)
    ...
    def audit(
        ...
        with self.connect() as conn:
            cur = conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)""",
                (action, actor, target_type, target_id, details_json, utc_now()),
            )
            return int(cur.lastrowid)
